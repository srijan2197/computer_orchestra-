"""
AI layer — two models, zero external dependencies:
  1. Holt Double EMA      → real-time demand forecasting (online learning)
  2. Z-score Anomaly Det. → spike / fault detection per resource
"""
...
class DemandPredictor:
    def __init__(self, resources: List[str], alpha: float = 0.4, beta: float = 0.3, window: int = 40):
        ...
        self.anomalies: Dict[str, bool] = {r: False for r in resources}
    ...
    def update(self, observed: Dict[str, float]) -> Dict[str, float]:
        ...
        # Z-score anomaly detection — flag if latest value > 2.5 std devs from mean
        for r, value in observed.items():
            hist = list(self.history[r])
            if len(hist) >= 8:
                mean = sum(hist) / len(hist)
                std = (sum((x - mean) ** 2 for x in hist) / len(hist)) ** 0.5
                self.anomalies[r] = std > 0 and abs(value - mean) / std > 2.5
            else:
                self.anomalies[r] = False
